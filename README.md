...
* **What the project does:** The Online Shopping Intent Predictor is an end-to-end web application built with Flask that evaluates online shopping behavior and purchase intent in real time. It takes customer session features—such as page values, product-related duration, bounce rates, visit month, and visitor type—processes them through a trained classification model and scaler, and instantly outputs a purchase prediction accompanied by confidence scoring.
* **The problem it solves:** E-commerce platforms often struggle to identify high-conversion user sessions dynamically before visitors leave the site. This project addresses this challenge by automating the user intent risk and conversion assessment pipeline using predictive analytics. It replaces guesswork with data-driven consistency while delivering instant feedback via a clean web interface.
...
* **Interactive Web Interface:** A clean, responsive form built with HTML/CSS integrated into Flask that allows users to easily input session metrics, month selections, and visitor characteristics.
* **Real-Time Predictive Analytics:** Instant purchase intention classification powered by a trained Scikit-Learn Logistic Regression model (`logistic_regression_model.pkl`).
* **Feature Normalization & Alignment:** Dynamic scaling utilizing a pre-trained `scaler.pkl` pipeline combined with automated one-hot encoded feature mapping (`model_columns.pkl`) to guarantee precise inference alignment.
* **Confidence Scoring Feedback:** Dynamic result blocks that display exact probability confidence metrics alongside purchase intent decisions.

## 🛠 Tech Stack & Dependencies

* **Programming Language:** Python
* **Web Framework:** Flask (for serving web routes and handling POST requests)
* **Machine Learning Model:** Scikit-Learn (Logistic Regression classifier and StandardScaler)
* **Model Persistence & Handling:** Joblib, Pandas, and NumPy
* **Frontend Template:** HTML5 & CSS3 with Jinja2 templating

## 📂 Project Structure

```text
├── app.py                         # Main Flask web application backend script
├── logistic_regression_model.pkl  # Pre-trained Logistic Regression machine learning model
├── scaler.pkl                     # Pre-trained StandardScaler object for normalization
├── model_columns.pkl              # Saved list of trained feature columns for alignment
├── requirements.txt               # List of required Python packages and dependencies
├── templates/
│   └── index.html                 # HTML frontend web interface form
...
Flask will launch a local server (typically at `[http://127.0.0.1:5000/](http://127.0.0.1:5000/)`). Open this link in your web browser.
